...
def merge_traceability_from_env(
    ...
    2. ``METALLB_*`` or ``BOND_CNI_*`` convenience keys when the corresponding ``POLARION_TRACE_*`` was **not** set.
    3. Values already in ``base`` (from the epic module).

    ``METALLB_JIRA_EPIC_KEY`` / ``BOND_CNI_JIRA_EPIC_KEY`` sets ``epic_label`` and, unless ``POLARION_TRACE_EPIC_URL`` is set,
    ``epic_url`` as ``{browse_base}/{key}`` (default browse base is Red Hat issues).
    """
    out = dict(base)
    ...
    browse_base = (
        ...
        or env.get("BOND_CNI_JIRA_BROWSE_URL_BASE", "https://issues.redhat.com/browse")
    ).rstrip("/")
    if epic_key:
    ...
    hl = env.get("METALLB_HIGH_LEVEL_PLAN_URL", "").strip() or env.get("BOND_CNI_HIGH_LEVEL_PLAN_URL", "").strip()
    if hl and not env.get("POLARION_TRACE_HIGH_LEVEL_PLAN_URL", "").strip():
        out["high_level_plan_url"] = hl

    dl = env.get("METALLB_DETAILED_PLAN_URL", "").strip() or env.get("BOND_CNI_DETAILED_PLAN_URL", "").strip()
