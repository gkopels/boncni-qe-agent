...
All areas follow **Phase 1 → 2 → 3 → 4** with **mandatory user validation** between phases. Full gates: MetalLB → `metallb-qe-lifecycle.mdc`; Bond CNI → `bond-cni-qe-lifecycle.mdc`.
...
2. **Detailed test plan** — From approved high-level Doc; optional `KUBECONFIG` to validate every TC; OCPBUGS with domain component (**Networking / Metal LB** or **Networking / Bond CNI**); user approves Doc; then Polarion LiveDoc under **`CNF`** unless overridden.
...
| ----- | ---------------- |
| **Jira Epic key** | User message → `METALLB_JIRA_EPIC_KEY` / `BOND_CNI_JIRA_EPIC_KEY` in `.env` / `export` (match the active skill set) → ask once if still missing. |
